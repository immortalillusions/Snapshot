...
export async function GET(request: Request) {
  ...
  if (existing.rows[0]) {
    ...
    await pool.query("update users set email = $1, avatar_url = $2, access_token = $3, refresh_token = coalesce($4, refresh_token), updated_at = now() where id = $5", [profile.data.email, profile.data.picture ?? null, tokens.access_token, tokens.refresh_token, userId]);

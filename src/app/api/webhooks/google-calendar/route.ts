/** Validates Google Calendar notifications and reconciles changed events. */
...
/** Provides a lightweight health response for the webhook endpoint. */
export function GET() {
	return NextResponse.json({ ok: true, service: "google-calendar-webhook" });
}

/** Reconciles a verified push notification and always acknowledges delivery. */
export async function POST(request: Request) {
	...
	if (!userId || !channelId || !resourceId) {
		return new NextResponse(null, { status: 204 });
	}
	const result = await pool.query(
		"select 1 from calendar_sync_state where user_id = $1 and channel_id = $2 and channel_resource_id = $3",
		[userId, channelId, resourceId],
	);
	if (!result.rowCount) return new NextResponse(null, { status: 204 });
	if (resourceState === "sync") {
		console.info("Google Calendar watch confirmed", {
			userId,
			channelId,
			resourceId,
		});
	...
	try {
		...
		await regenerateSummaries(userId, new Date(), {
			weeklyWeekStarts: syncResult.weeklyWeekStarts,
		});
		console.info("Google Calendar webhook summary regeneration complete", {
			userId,
		});
	} catch (error: unknown) {
